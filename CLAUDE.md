...
El widget "Top Products" del Home se rediseñó DOS veces: el podio de cristal del 2026-07-24 (elegido por mockup entre 3 opciones) le terminó pareciendo desorganizado al usuario ("5 capas de texto apiladas por columna a tamaños distintos"), y el 2026-07-27 se reemplazó — tras otra ronda de 3 mockups — por una **tabla zebra tipo documento**: el mismo lenguaje visual de las facturas (encabezado con regla `border-b-2 border-foreground`, filas alternadas `bg-secondary/40`, números tabulares alineados a la derecha). Columnas: medalla/rank (top 3 = circulito dorado/verde/gris del tema; 4+ número plano), Product (foto miniatura + nombre uppercase + SKU mono gris), Qty, Revenue. Sin barras de progreso — `calcTopProductos`/`top15` (la lógica de datos) no cambió, solo el markup. **Top 3 destacado (mismo día, segundo ajuste)**: fondo con lavado del color de su medalla (`color-mix(in srgb, <color> 8%, transparent)`), foto más grande (w-10 vs w-8), nombre en bold y monto en 13px (el anillo de color en la foto del top 3 se probó y el usuario lo pidió quitar — todas las fotos llevan un marco gris uniforme `border-2 border-muted-foreground/30`); la zebra corre solo del 4to en adelante (`(i - 3) % 2 === 1`). Las variables de glassmorphism (`--glass` etc.) siguen definidas en globals.css pero ya no las usa este widget.
