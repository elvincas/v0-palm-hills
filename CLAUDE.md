...
Facturas y estimates guardan `precioCatalogo` por línea (precio de catálogo puro, sin lista) además de `precio`/`precioOriginal`. En `/facturas/[id]` y `/ordenes/[id]/estimado` hay dos switches (ambos encendidos por defecto):
- **"Show discounts"** (maestro, 2026-07-27): aparece si hay CUALQUIER descuento que ocultar — ajuste manual por línea O descuento de lista. Apagado, el documento queda limpio: solo precios finales, sin tachados ni línea "Discount" (Subtotal = Total). Antes el ajuste manual se mostraba tachado siempre sin forma de ocultarlo (pedido del usuario a raíz de la orden #81).
