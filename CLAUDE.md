...
### Catálogo de productos (2026-07-21, rediseño 2026-07-24)
...
- **Header/footer en las hojas de producto**: header con logo chico + nombre + "Product Catalog — {almacén}" a la izquierda, "CATALOG" + fecha a la derecha (como antes, pero ahora con logo). Footer nuevo (`fixed`, con `render={({pageNumber, totalPages}) => ...}` igual que documento-pdf.tsx): nombre + cantidad + fecha a la izquierda, "Page X of Y" a la derecha — antes no había numeración de página en ningún lado.
- **Tarjetas del grid** (con fotos): SKU como pill flotante sobre la foto (esquina superior izquierda), nombre en bold, `fabricante` (marca) como subtítulo si existe — es el campo mas cercano a "descripción" que tiene `productos` (no hay un campo de texto libre de descripción en el modelo de datos), precio como badge sólido verde en vez de texto plano.
- **Tabla** (sin fotos): ganó una columna "Brand" (`fabricante`), header con banda de fondo verde sólido y texto blanco (antes era una línea con borde inferior nomás), filas alternadas con el verde-claro `#eef5e6` de siempre (antes gris genérico).
...
- `ProductoCatalogo`/`DatosCatalogo` ganaron `fabricante`, `empresaContacto`, `logo` — todos opcionales, así que llamadas viejas (si las hubiera) no se rompen.
