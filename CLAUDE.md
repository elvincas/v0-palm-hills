...
### Top Products — podio de cristal (2026-07-24)
El widget "Top Products" del Home (antes tarjeta plana, ver captura del usuario) se rediseñó a pedido explícito tras aprobar un mockup (3 opciones: podio de cristal, lista minimalista, leaderboard con barras — eligió la primera). Cambios sobre `calcTopProductos`/`top15` (la lógica de datos NO cambió, solo el markup):
- Contenedor: `var(--glass)` + `backdrop-blur-xl` + `var(--glass-border)` en vez de `bg-card border-border` plano.
- Podio: orden visual 2do–1er–3er lugar (el 1ro con `-mt-3` para verse elevado), medalla como circulito de color sólido (dorado/verde/gris) flotando en la esquina de la foto en vez del emoji 🥇🥈🥉, sombra con glow del color de cada posición (`color-mix(in srgb, ...)`).
- Posiciones 4–15: el número de rango pasa de texto plano a un chip circular de vidrio (`var(--glass-strong)` + borde interior sutil).
- Todo restyleado con variables de tema (`var(--glass)`, `var(--primary)`, etc.) — se ve correcto en claro y oscuro sin código condicional por tema.
